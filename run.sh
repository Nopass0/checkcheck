...
# Функция для остановки всех связанных процессов
cleanup_all() {
    echo -e "${YELLOW}🛑 Остановка всех связанных процессов...${NC}"
    
    # Останавливаем все экземпляры бота
    pkill -f "python bot.py" >/dev/null 2>&1
    
    # Останавливаем все экземпляры Next.js
    pkill -f "next dev\|next start" >/dev/null 2>&1
    
    # Освобождаем порт 6060
...
# Очищаем все старые процессы перед запуском
