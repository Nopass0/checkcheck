...
2. Дата и время
3. Сумма перевода (для альфы с пробелом между тысячами: 14 900)
4. Отправитель - для альфы не указывается
5. Номер телефона (для альфы без форматирования: 79055555536, для т банка: +7 (985) 535-25-11)
6. Получатель (для альфы формат: Имя Отчество Инициал, например: Ильман Тагирович Д)
7. Банк получателя
8. Идентификатор операции (для альфы - идентификатор операции в СБП)
9. Номер квитанции (для альфы - номер операции)
10. Счет списания
11. Сообщение получателю (только для альфы, опционально)

Всего включая банк отправителя:
- 9-10 строк для альфы (с сообщением или без)
- 10 строк для т банка"""
...
def normalize_text_alfa(text):  # приведение текста к единообразному виду
    ...
    # Нормализуем телефон для Альфа Банка - убираем все кроме цифр
    phone = text["pfone_number"].strip()
    ...
    # Если не начинается с 7, добавляем 7
        ...
        phone = '7' + phone
    normalized_text["pfone_number"] = phone

    normalized_text["recipient"] = text["recipient"].strip()
    normalized_text["bank"] = text["bank"].strip()
    normalized_text["operation_id"] = text["operation_id"].strip()
    normalized_text["receipt_number"] = text["receipt_number"].strip()
    normalized_text["card_number"] = text["card_number"].strip()

    # Добавляем сообщение если оно есть
    if "message" in text:
        normalized_text["message"] = text["message"].strip()
...
def generate_file(text):  # создание файла
    ...
    if from_bank == "т банк":
        ...
        # Для Альфа Банка может быть 9 или 10 строк (с сообщением или без)
        if len(rows) not in [9, 10]:
            return {"status": "error", "error": "Неверное количество строк"}
        new_text = {
            "from_bank": from_bank,
            "date": rows[1],
