...
3. **Запустите приложение:**
   ...
   **Полное приложение (бот + фронтенд):**
   ```bash
   ./run.sh              # разработка
   ./run.sh prod         # продакшен
   ./run-production.sh   # быстрый продакшен
   ```
   
   **Только фронтенд (если бот конфликтует):**
   ```bash
   ./run-frontend-only.sh         # разработка
   ./run-frontend-only.sh prod    # продакшен
   ```
...
- **Откроет порт 6060 в файерволе** (UFW/firewalld/iptables)
- Запустит Telegram бота в виртуальном окружении
- Запустит веб-приложение на http://localhost:6060 (доступно извне)
...
- Доступен только локально и в сети
...
- Доступен на всех сетевых интерфейсах (0.0.0.0)
- Автоматически открывает порты в файерволе
- Стабильная работа для продакшена
...
```bash
cd frontend
PORT=6060 bun run dev-prod    # доступен извне
bun run dev-no-turbo          # без Turbopack
```
...
bun run start-prod           # запуск продакшен сервера
```
